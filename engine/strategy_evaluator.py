...
class StrategyEvaluator:
    ...
    def score_strategy(self, metrics: Dict[str, Any]) -> float:
        ...
        try:
            ...
            component_scores["returns"] = min(
                1.0, max(0, ann_return / 0.30)
            )  # Cap at 30% annual return

            # Drawdown: Lower is better
            max_dd = metrics.get("max_drawdown", 1.0)  # Default to worst case
            component_scores["drawdown"] = max(
                0, 1.0 - (max_dd / 0.20)
            )  # Scale so 20% drawdown = 0

            # Consistency metrics
            win_rate = metrics.get("win_rate", 0)
            component_scores["win_rate"] = min(
                1.0, max(0, win_rate / 0.65)
            )  # Scale to 65% win rate = 1.0
            ...
            # Calc weighted score
            final_score = sum(
                score * self.weights[metric] for metric, score in component_scores.items()
            )
