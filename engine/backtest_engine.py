...
class BacktestEngine:
    ...
    async def run_backtest(
        ...
        logger.info(f"Loading historical data for {len(symbols)} symbols from {start_dt.date()} to {end_dt.date()}...")
        ...
        for day_num, current_date in enumerate(trading_days):
            try:
                ...
                current_date_utc = current_date.replace(tzinfo=pytz.UTC) if current_date.tzinfo is None else current_date
                ...
                if day_num % 50 == 0:
                    logger.info(f"  Day {day_num}/{len(trading_days)}: Equity = ${portfolio_value:,.2f}")
