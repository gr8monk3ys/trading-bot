...
def print_dashboard(stats, iteration):
    ...
    print(
        f"    Total P&L:        {pl_color}${stats['total_pl']:>12,.2f} ({stats['total_pl_pct']:>6.2f}%){reset_color}"
    )
    ...
    if stats["position_details"]:
        ...
        print(
            f"    {'Symbol':<8} {'Qty':<10} {'Entry':<12} {'Current':<12} {'P&L':<14} {'P&L %':<10}"
        )
        print("    " + "-" * 80)
        for pos in stats["position_details"]:
            ...
            print(
                f"    {pos.symbol:<8} {qty:<10.2f} ${entry:<11.2f} ${current:<11.2f} "
