...
class BracketMomentumStrategy(BaseStrategy):
    ...
    async def initialize(self, **kwargs):
        ...
        try:
            ...
            logger.info(f"Profit target: {self.profit_target_pct:.1%}, Stop loss: {self.stop_loss_pct:.1%}")
            return True
    ...
    async def on_bar(self, symbol, open_price, high_price, low_price, close_price, volume, timestamp):
        ...
        try:
            ...
            max_history = max(self.slow_ma, self.rsi_period, self.macd_slow + self.macd_signal, self.atr_period) + 10
    ...
    async def _generate_signal(self, symbol):
        ...
        try:
            ...
            buy_signal = (
                rsi < self.rsi_buy_threshold and  # RSI low but not extreme
                macd > macd_signal and  # MACD bullish crossover
                macd_hist > 0 and  # MACD histogram positive
                fast_ma > slow_ma  # Fast MA above slow MA (uptrend)
            )
    ...
    async def _execute_bracket_buy(self, symbol):
        ...
        try:
            ...
            if result:
                ...
                logger.info(f"   Risk/Reward: {self.stop_loss_pct:.1%} / {self.profit_target_pct:.1%}")
