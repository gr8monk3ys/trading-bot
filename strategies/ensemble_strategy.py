...
class EnsembleStrategy(BaseStrategy):
    ...
    async def initialize(self, **kwargs):
        ...
        try:
            ...
            self.market_regime = {symbol: 'unknown' for symbol in self.symbols}
            self.sub_strategy_signals = {symbol: {} for symbol in self.symbols}
            self.ensemble_signals = {symbol: 'neutral' for symbol in self.symbols}
            self.current_prices = {}
    ...
    async def _update_indicators(self, symbol):
        ...
        try:
            ...
            self.indicators[symbol] = {
                ...
                'fast_ma': fast_ma[-1] if len(fast_ma) > 0 and not np.isnan(fast_ma[-1]) else closes[-1],
                'slow_ma': slow_ma[-1] if len(slow_ma) > 0 and not np.isnan(slow_ma[-1]) else closes[-1],
                'ema_20': ema_20[-1] if len(ema_20) > 0 and not np.isnan(ema_20[-1]) else closes[-1],
                ...
                # Volatility
                'bb_upper': bb_upper[-1] if len(bb_upper) > 0 and not np.isnan(bb_upper[-1]) else closes[-1] * 1.02,
                'bb_middle': bb_middle[-1] if len(bb_middle) > 0 and not np.isnan(bb_middle[-1]) else closes[-1],
                'bb_lower': bb_lower[-1] if len(bb_lower) > 0 and not np.isnan(bb_lower[-1]) else closes[-1] * 0.98,
                ...
                'volume': volumes[-1],
                'volume_sma': volume_sma[-1] if len(volume_sma) > 0 and not np.isnan(volume_sma[-1]) else volumes[-1],
    ...
    async def _generate_sub_strategy_signals(self, symbol):
        ...
        try:
            ...
            if macd > macd_signal_line and macd_hist > 0 and adx > self.parameters['mom_adx_threshold']:
                mom_score = 1
            elif macd < macd_signal_line and macd_hist < 0 and adx > self.parameters['mom_adx_threshold']:
                mom_score = -1
    ...
    async def _combine_signals(self, symbol):
        ...
        try:
            ...
            if buy_agreement >= min_agreement:
                ...
                logger.debug(f"{symbol} ENSEMBLE BUY (agreement: {buy_agreement:.1%}, regime: {regime})")
            elif sell_agreement >= min_agreement:
                self.ensemble_signals[symbol] = 'sell'
                logger.debug(f"{symbol} ENSEMBLE SELL (agreement: {sell_agreement:.1%}, regime: {regime})")
            else:
                self.ensemble_signals[symbol] = 'neutral'
