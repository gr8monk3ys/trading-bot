...
async def run_backtest_dashboard():
    ...
    logger.info(
        f"Running momentum strategy backtest from {start_date.date()} to {end_date.date()}..."
    )
    momentum_backtest = BacktestEngine(
        strategy=momentum, start_date=start_date, end_date=end_date, symbols=test_symbols
    )
    momentum_results = await momentum_backtest.run()

    # Run mean reversion strategy backtest
    logger.info(
        f"Running mean reversion strategy backtest from {start_date.date()} to {end_date.date()}..."
    )
    mean_reversion_backtest = BacktestEngine(
        strategy=mean_reversion, start_date=start_date, end_date=end_date, symbols=test_symbols
    )
    ...
    benchmark_data = pd.DataFrame(
        {"equity": benchmark_equity, "returns": pd.Series(daily_returns)}, index=benchmark_dates
    )
    ...
    # Plot equity curves
    momentum_results["equity_curve"]["equity"].plot(ax=ax, linewidth=2, label="Momentum Strategy")
    mean_reversion_results["equity_curve"]["equity"].plot(
        ax=ax, linewidth=2, label="Mean Reversion Strategy"
    )
    benchmark_data["equity"].plot(ax=ax, linewidth=2, linestyle="--", label="Benchmark (S&P 500)")
