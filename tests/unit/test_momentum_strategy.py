...
class TestUpdateIndicators:
    ...
    async def test_update_indicators_insufficient_history(self):
        ...
        strategy.indicators = {'AAPL': {}}
        strategy.price_history = {'AAPL': [
            {'close': 150, 'high': 151, 'low': 149, 'volume': 1000000}
        ]}
...
class TestGenerateSignal:
    ...
    @pytest.mark.asyncio
    async def test_generate_signal_mtf_strict_rejects_bearish_on_bullish(self, strategy_with_indicators):
...
class TestOnBar:
    ...
    async def test_on_bar_ignores_unknown_symbol(self, initialized_strategy):
        ...
        await initialized_strategy.on_bar(
            'UNKNOWN', 100, 101, 99, 100.5, 1000000, datetime.now()
        )
    ...
    async def test_on_bar_stores_current_price(self, initialized_strategy):
        ...
        await initialized_strategy.on_bar(
            'AAPL', 150, 152, 149, 151, 1000000, datetime.now()
        )
    ...
    async def test_on_bar_updates_price_history(self, initialized_strategy):
        ...
        await initialized_strategy.on_bar(
            'AAPL', 150, 152, 149, 151, 1000000, timestamp
        )
...
class TestExecuteSignal:
    ...
    async def test_execute_signal_uses_kelly_criterion(self, trading_strategy):
        ...
        trading_strategy.calculate_kelly_position_size = AsyncMock(
            return_value=(15000, 0.15, 100)
        )
...
class TestCheckExitConditions:
    ...
    async def test_check_exit_conditions_handles_exception(self, strategy_with_position):
        ...
        strategy_with_position.broker.get_positions = AsyncMock(
...
class TestGenerateSignals:
    ...
    def backtest_strategy(self):
        ...
        np.random.seed(42)
        dates = pd.date_range(end=datetime.now(), periods=100, freq='min')  # 'min' instead of deprecated 'T'
        data = pd.DataFrame({
            ...
            'volume': (np.random.randint(900000, 1100000, 100)).astype(np.float64)  # Must be float64 for talib
        }, index=dates)
