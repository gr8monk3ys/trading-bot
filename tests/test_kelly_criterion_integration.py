...
class TestKellyParameters:
    ...
    def test_kelly_disabled_by_default(self):
        """Kelly Criterion should be disabled by default until validated with sufficient trades."""
        from strategies.momentum_strategy import MomentumStrategy

        strategy = MomentumStrategy()
        params = strategy.default_parameters()

        # Kelly is an experimental feature - disabled by default until 100+ trades validate it
