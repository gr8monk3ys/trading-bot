            ...
            df = pd.DataFrame(
                {
                    ...
                    "open": prices,
                    "high": [
                        p * (1 + np.random.uniform(0, 0.02)) for p in prices
                    ],  # High is up to 2% above open
                    "low": [
                        p * (1 - np.random.uniform(0, 0.02)) for p in prices
                    ],  # Low is up to 2% below open
                    "close": [
                        p * (1 + np.random.normal(0, 0.005)) for p in prices
                    ],  # Close is normally distributed around open
                    "volume": [
                        int(np.random.uniform(100000, 1000000)) for _ in prices
                    ],  # Random volume
                }
            )
...
class BacktestSimulator:
    ...
    def __init__(
        self, strategy_class, symbols, start_date, end_date, initial_capital=100000, timeframe="1D"
    ):
    ...
    async def run_backtest(self, strategy_params=None):
        ...
        try:
            ...
            for date in dates:
                ...
                self.portfolio_values.append({"date": date.date(), "value": account.equity})
                ...
                if len(self.portfolio_values) > 1:
                    ...
                    self.daily_returns.append({"date": date.date(), "return": daily_return})
    ...
    def _calculate_performance(self):
        ...
        wins = [
            ...
            if (t["side"] == "buy" and t["price"] < portfolio_series.iloc[-1])
            or (t["side"] == "sell" and t["price"] > portfolio_series.iloc[-1])
        ]
...
async def test_strategy(args):
    ...
    try:
        ...
        # Get symbols
        symbols = (
            args.symbols.split(",") if args.symbols else ["AAPL", "MSFT", "GOOGL", "AMZN", "TSLA"]
        )
        ...
        if args.strategy not in strategy_classes:
            print(
                f"Strategy '{args.strategy}' not available. Choose from: {list(strategy_classes.keys())}"
            )
            return
        ...
        if args.plot:
            try:
                ...
                if result["trades"]:
                    ...
                    print(
                        f"Equity curve with trades saved as {args.strategy}_equity_curve_with_trades.png"
                    )
    ...
    async def run_comparison():
        try:
            ...
            strategies_to_compare = (
                args.strategies.split(",") if args.strategies else list(strategy_classes.keys())
            )
            ...
            if len(results) > 1:
                ...
                print(
                    comparison.to_string(
                        float_format=lambda x: f"{x:.2%}" if x < 10 else f"{x:.2f}"
                    )
                )
...
def main():
    ...
    test_parser.add_argument(
        "--symbols", default=None, help="Comma-separated list of symbols to trade"
    )
    test_parser.add_argument(
        "--capital", type=float, default=100000, help="Initial capital for test"
    )
    test_parser.add_argument("--params", default=None, help="JSON string with strategy parameters")
    test_parser.add_argument(
        "--plot", action="store_true", help="Generate a plot of the equity curve"
    )

    # Compare command
    compare_parser = subparsers.add_parser("compare", help="Compare multiple strategies")
    compare_parser.add_argument(
        "--strategies", default=None, help="Comma-separated list of strategies to compare"
    )
    compare_parser.add_argument(
        "--start-date",
        default=(datetime.now() - timedelta(days=90)).strftime("%Y-%m-%d"),
    ...
    compare_parser.add_argument(
        "--plot", action="store_true", help="Generate a plot comparing equity curves"
    )
