...
class TestSentimentAnalysis(unittest.TestCase):
    ...
    def test_positive_sentiment(self):
        ...
        for text in self.positive_texts:
            ...
            self.assertEqual(
                sentiment, "positive", "Failed to detect positive sentiment in: " + text
            )
            self.assertGreater(probability, 0, "Probability should be > 0 for positive sentiment")
    ...
    def test_negative_sentiment(self):
        ...
        for text in self.negative_texts:
            probability, sentiment = analyze_sentiment([text])
            self.assertEqual(
                sentiment, "negative", "Failed to detect negative sentiment in: " + text
            )
            self.assertGreater(probability, 0, "Probability should be > 0 for negative sentiment")
    ...
    def test_neutral_sentiment(self):
        ...
        for text in self.neutral_texts:
            ...
            self.assertGreater(probability, 0, "Probability should be > 0 for neutral sentiment")

    def test_empty_input(self):
        """Test empty input handling."""
        probability, sentiment = analyze_sentiment([])
        self.assertEqual(sentiment, "neutral", "Empty input should return neutral sentiment")
        self.assertEqual(probability, 0, "Empty input should return 0 probability")

    def test_multiple_texts(self):
        """Test sentiment analysis with multiple texts."""
        mixed_texts = [self.positive_texts[0], self.negative_texts[0], self.neutral_texts[0]]
        probability, sentiment = analyze_sentiment(mixed_texts)
        self.assertIn(
            sentiment, ["positive", "negative", "neutral"], "Invalid sentiment for multiple texts"
        )
        self.assertGreater(probability, 0, "Probability should be > 0 for multiple texts")
