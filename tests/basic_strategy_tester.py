...
class SimpleBacktester:
    ...
    def _generate_synthetic_data(self, symbol):
        ...
        daily_return = (1 + annual_return) ** (1/252) - 1  # Convert to daily return (252 trading days)
        ...
        df = pd.DataFrame({
            'date': dates,
            'open': prices,
            'high': [p * (1 + random.uniform(0, 0.02)) for p in prices],  # High up to 2% above open
            'low': [p * (1 - random.uniform(0, 0.02)) for p in prices],   # Low up to 2% below open
            'close': [p * (1 + random.normalvariate(0, 0.005)) for p in prices],  # Close normally distributed around open
    ...
    def run_momentum_strategy(self, fast_ma=12, slow_ma=26, rsi_period=14, rsi_oversold=30, rsi_overbought=70):
        """Run a basic momentum strategy."""
        logger.info(f"Running momentum strategy (Fast MA: {fast_ma}, Slow MA: {slow_ma}, RSI: {rsi_period})")
        ...
        for date in dates:
            ...
            for symbol in self.symbols:
                ...
                if prev_data is not None:
                    ma_crossover_bullish = (current_data['fast_ma'] > current_data['slow_ma'] and 
                                          prev_data['fast_ma'] <= prev_data['slow_ma'])
                    ma_crossover_bearish = (current_data['fast_ma'] < current_data['slow_ma'] and 
                                          prev_data['fast_ma'] >= prev_data['slow_ma'])
                    
                    # RSI conditions
                    rsi_oversold_bullish = (current_data['rsi'] < rsi_oversold and 
                                         prev_data['rsi'] >= rsi_oversold)
                    rsi_overbought_bearish = (current_data['rsi'] > rsi_overbought and 
                                           prev_data['rsi'] <= rsi_overbought)
                ...
                if buy_signal and position <= 0:
                    ...
                    if shares_to_buy > 0 and self.current_capital >= shares_to_buy * current_price:
                        ...
                        logger.info(f"{date}: BUY {shares_to_buy} shares of {symbol} at ${current_price:.2f}")
                    ...
                    if shares_to_sell > 0:
                        ...
                        logger.info(f"{date}: SELL {shares_to_sell} shares of {symbol} at ${current_price:.2f}")
    ...
    def run_mean_reversion_strategy(self, lookback_period=20, entry_z=1.5, exit_z=0.5):
        ...
        logger.info(f"Running mean reversion strategy (Lookback: {lookback_period}, Entry Z: {entry_z}, Exit Z: {exit_z})")
            ...
            for symbol in self.symbols:
                ...
                if buy_signal and position <= 0:
                    ...
                    if shares_to_buy > 0 and self.current_capital >= shares_to_buy * current_price:
                        ...
                        logger.info(f"{date}: BUY {shares_to_buy} shares of {symbol} at ${current_price:.2f} (z-score: {z_score:.2f})")
                    ...
                    if shares_to_sell > 0:
                        ...
                        logger.info(f"{date}: SELL {shares_to_sell} shares of {symbol} at ${current_price:.2f} (z-score: {z_score:.2f})")
    ...
    def _calculate_performance(self, final_portfolio_value):
        ...
        if not portfolio_df.empty and len(portfolio_df) > 1:
            ...
            portfolio_df['drawdown'] = (portfolio_df['cum_max'] - portfolio_df['value']) / portfolio_df['cum_max']
        ...
        if self.trades:
            ...
            for buy in buy_trades:
                ...
                matching_sells = [s for s in sell_trades if s['symbol'] == symbol and s['date'] > buy_date]
                if matching_sells:
    ...
    def compare_strategies(self, plot=True):
        ...
        return {'momentum': momentum_result, 'mean_reversion': mean_reversion_result, 'comparison': comparison}
            ...
            if not trades_df.empty:
                ...
                for _, buy in buys.iterrows():
                    if buy['date'] in portfolio_dict:
                        plt.scatter(buy['date'], portfolio_dict[buy['date']], 
                                   marker='^', color='green', s=100)
                
                # Plot sell points
                for _, sell in sells.iterrows():
                    if sell['date'] in portfolio_dict:
                        plt.scatter(sell['date'], portfolio_dict[sell['date']], 
...
def main():
    ...
    parser.add_argument('--symbols', default='AAPL,MSFT,GOOGL,AMZN',
