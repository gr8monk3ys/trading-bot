...
class MockAlpacaBroker:
    ...
    async def get_account(self) -> MockAccount:
        """Get account info"""
        position_value = sum(p.qty * p.current_price for p in self.positions.values())
    ...
    async def submit_order(
        self, symbol: str, qty: float, side: str, type: str = "market", time_in_force: str = "day"
    ) -> MockOrder:
    ...
    async def _execute_order(self, order: MockOrder):
        ...
        if side == "buy":
            ...
            if symbol in self.positions:
                ...
                pos.unrealized_plpc = (
                    (price - new_avg_price) / new_avg_price if new_avg_price > 0 else 0
                )
            else:
    ...
    async def _generate_price_data(
        self, symbol: str, start: datetime, end: datetime, market_regime: str = "normal"
    ) -> List[MockBar]:
        """
        ...
        returns = np.random.normal(params["drift"], params["volatility"], num_bars)
        ...
        for i, date in enumerate(dates):
            ...
            bars.append(
                MockBar(
                    timestamp=date, open=open_price, high=high, low=low, close=close, volume=volume
                )
            )
