...
async def test_momentum_signal_generation(momentum_strategy, test_symbols, mock_broker):
    ...
    price_data = await mock_broker.get_bars(test_symbols, "day", start_date, end_date)
...
async def test_position_sizing(momentum_strategy, test_symbols, mock_broker):
    ...
    # Test position sizing logic
    expected_size = round(
        mock_broker.account["portfolio_value"] * momentum_strategy.parameters["position_size"]
    )
