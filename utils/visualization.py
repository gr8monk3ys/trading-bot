...
def plot_drawdown_periods(equity_curve: pd.DataFrame, top_n: int = 5) -> Figure:
    ...
    for i, (_, period) in enumerate(top_periods.iterrows()):
        ...
        ax.plot(period_equity.index, period_equity.values, 
        ...
        if recovery_idx:
            ax.scatter(recovery_idx, equity_curve.loc[recovery_idx, 'equity'], color=colors[i], marker='o', s=100)
...
def plot_trade_analysis(trades: List[Dict]) -> Tuple[Figure, Figure]:
    ...
    if 'profit_loss_pct' not in trade_df.columns and 'entry_price' in trade_df.columns and 'exit_price' in trade_df.columns:
    ...
    if 'duration' not in trade_df.columns and 'entry_time' in trade_df.columns and 'exit_time' in trade_df.columns:
        trade_df['duration'] = (pd.to_datetime(trade_df['exit_time']) - pd.to_datetime(trade_df['entry_time'])).dt.days
    ...
    axes1[0, 0].pie([winners, losers], labels=['Winners', 'Losers'], autopct='%1.1f%%', colors=['green', 'red'])
    ...
    sns.scatterplot(data=trade_df, x='duration', y='profit_loss_pct', 
                    hue='side' if 'side' in trade_df.columns else None, ax=axes2[1])
