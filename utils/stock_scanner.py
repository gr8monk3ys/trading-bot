...
class StockScanner:
    ...
    def calculate_technical_score(self, data: pd.DataFrame) -> Tuple[float, Dict]:
        ...
        try:
            ...
            volume_score = (0.5 * (1 if volume[-1] > np.mean(volume[-20:]) else 0)) + (
            ...
            metrics = {
                ...
                "volume": {"mfi": current_mfi, "volume_change": volume[-1] / np.mean(volume[-20:])},
                "risk": {
                ...
                "support_resistance": {"s2": s2, "s1": s1, "pivot": pivot, "r1": r1, "r2": r2},
            }
    ...
    ) -> Tuple[pd.DataFrame, Tuple[float, Dict]]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    logger.error(
                        f"Error getting market data for {symbol} after {max_retries} attempts: {e}"
                    )
                    return None, (0, {})
    ...
    def get_sentiment_score(self, symbol: str, max_retries: int = 3) -> float:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    logger.error(
                        f"Error getting sentiment for {symbol} after {max_retries} attempts: {e}"
                    )
                    return 0.5
                logger.warning(f"Attempt {attempt + 1} failed for {symbol}, retrying...")
                continue
    ...
    def scan_single_stock(self, symbol: str) -> Dict:
        ...
        try:
            ...
            combined_score = ((technical_score * 0.6) + (sentiment_score * 0.4)) * risk_adjustment
