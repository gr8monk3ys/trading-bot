...
def _format_markdown(artifact: dict) -> str:
    ...
    if inconclusive:
        ...
        interpretation += (
            ...
            "2. **PnL accounting under-reports per-trade P&L.** The engine's "
            "trade-matching logic only assigns PnL to sells that follow buys in "
            "the same symbol; trades the strategy opens as shorts get `pnl: 0` "
            "and their gains/losses show up in equity but not in `avg_trade` / "
            "`profit_factor`. Treat trade-level stats as lower bounds.\n"
            "3. **Mark-to-market dominates the headline return.** A large fraction "
            "of the final equity sits in still-open positions on 2024-12-31, "
            "valued at end-of-period prices. The 5-year window happens to end "
            "near all-time highs; rerun ending on a different date for a "
            "different number.\n"
            "4. **Costs included: 40 bps slippage + 10 bps spread per trade.** "
            ...
            "5. **No walk-forward validation in this artifact.** This is a single "
