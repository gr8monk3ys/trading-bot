...
class MockMomentumStrategy:
    ...
    def get_signal(self, symbol, bar_data):
        ...
        if symbol in self.positions and self.positions[symbol] > 0:
            ...
            if entry_price > 0:
                ...
                if pnl_pct < -self.stop_loss_pct:
                    ...
                    return {'side': 'sell', 'qty': qty, 'price': current_price, 'reason': 'stop_loss'}
                
                # Take profit - exit if gain exceeds threshold
                if pnl_pct > self.take_profit_pct:
                    qty = self.positions[symbol]
                    return {'side': 'sell', 'qty': qty, 'price': current_price, 'reason': 'take_profit'}
        ...
        try:
            ...
            # Generate signal
            if pd.notna(current_fast_ma) and pd.notna(current_slow_ma) and pd.notna(prev_fast_ma) and pd.notna(prev_slow_ma):
                # Buy signal: fast MA crosses above slow MA and RSI is not extreme
                if (prev_fast_ma <= prev_slow_ma and current_fast_ma > current_slow_ma) and current_rsi < 60 and active_positions < self.max_positions:
                    # Calculate position size (fixed percentage of portfolio)
                    qty = int(10000 / current_price)  # Simplified for mock testing
                    return {'side': 'buy', 'qty': qty, 'price': current_price, 'reason': 'momentum_crossover'}
                    
                # Sell signal: fast MA crosses below slow MA
                elif (prev_fast_ma >= prev_slow_ma and current_fast_ma < current_slow_ma) and symbol in self.positions:
                    qty = self.positions[symbol]
                    return {'side': 'sell', 'qty': qty, 'price': current_price, 'reason': 'momentum_reversal'}
                
                # Sell signal: RSI overbought
                elif current_rsi > self.rsi_overbought and symbol in self.positions:
                    qty = self.positions[symbol]
                    return {'side': 'sell', 'qty': qty, 'price': current_price, 'reason': 'overbought'}
    ...
    def get_signal(self, symbol, bar_data):
        ...
        try:
            ...
            if pd.notna(current_rsi) and pd.notna(current_lower) and pd.notna(current_upper):
                ...
                if ((current_price < current_lower and current_rsi < self.rsi_oversold) or 
                   (current_price < 0.95 * current_mean and current_rsi > 30 and current_rsi < 40)) and active_positions < self.max_positions:
                    # Calculate position size with volatility adjustment
                    volatility = rolling_std.iloc[-1] / current_price
                    position_size_adj = max(0.02, min(self.position_size, 0.05 / (volatility * 10))) # Reduce size for volatile assets
                ...
                # Sell signal: price reaches or crosses above mean after being below
                    ...
                    if current_price > entry_price * 0.98:
                        qty = self.positions[symbol]
                        return {'side': 'sell', 'qty': qty, 'price': current_price, 'reason': 'mean_reversion'}
