...
async def run_comparison(
    ...
    for i, config in enumerate(CONFIGURATIONS):
        ...
        try:
            ...
            if result:
                ...
                print(f"    ERROR: Backtest returned None")
    ...
    for r in results_sorted:
        ...
        print(f"{r['config']['name']:<20} {r['total_return']:>+9.2%} {r['sharpe_ratio']:>8.2f} {r['win_rate']:>9.1%} {r['max_drawdown']:>9.2%} {r['num_trades']:>8} {sig:>12}")
    ...
    if baseline:
        ...
        if improvements:
            ...
            for imp in sorted(improvements, key=lambda x: x['sharpe_diff'], reverse=True):
                print(f"   - {imp['name']}: Return {imp['return_diff']:+.2%}, Sharpe {imp['sharpe_diff']:+.2f}")
        else:
        ...
        if regressions:
            ...
            for reg in regressions:
                print(f"   - {reg['name']}: Return {reg['return_diff']:+.2%}, Sharpe {reg['sharpe_diff']:+.2f}")
